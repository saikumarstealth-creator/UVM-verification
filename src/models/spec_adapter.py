"""
Production-Grade Spec Adapter for UVM Testbench Generation.

Provides:
  - Schema validation at entry (both source and target specs)
  - Fuzzy signal/register matching with protocol-aware canonicalization
  - Error recovery orchestration (exact -> fuzzy -> structural -> LLM)
  - Adaptation preview with diff output
  - Rollback capability (undo adaptation if quality degrades)
  - Parallel adaptation across multiple strategies
  - Comprehensive confidence scoring with audit trail
"""
...
class AdaptationPreview:
    ...
    signal_changes: List[str] = field(default_factory=list)
    register_changes: List[str] = field(default_factory=list)
...
class AdaptationPlan:
    ...
    overall_score: float
    strategy_used: AdaptationStrategy = AdaptationStrategy.EXACT_MATCH
    preview: Optional[AdaptationPreview] = None
    warnings: List[str] = field(default_factory=list)
...
REQUIRED_SPEC_FIELDS = {"design_name", "interfaces", "registers"}
REQUIRED_SIGNAL_FIELDS = {"name", "direction"}
REQUIRED_REGISTER_FIELDS = {"name", "address", "access"}


class SchemaValidator:
    @staticmethod
    def validate_spec(spec_dict: Dict[str, Any], name: str = "spec") -> List[str]:
        errors = []
        ...
        for i, iface in enumerate(interfaces):
            if not isinstance(iface, dict):
                errors.append(f"{name}.interfaces[{i}]: not a dict")
                continue
            for s in iface.get("signals", []):
                for f in REQUIRED_SIGNAL_FIELDS:
                    if f not in s:
                        errors.append(f"{name}.interfaces[{i}].signal: missing '{f}'")
                if "width" not in s:
                    s["width"] = 1
        registers = spec_dict.get("registers", [])
        for i, reg in enumerate(registers):
            if not isinstance(reg, dict):
                errors.append(f"{name}.registers[{i}]: not a dict")
                continue
            for f in REQUIRED_REGISTER_FIELDS:
                if f not in reg:
                    errors.append(f"{name}.registers[{i}]: missing '{f}'")
            if "size" not in reg:
                reg["size"] = 8
...
class SignalCanonicalizer:
    ...
    @staticmethod
    def signature(name: str, direction: str, width: int, protocol: Optional[str] = None) -> str:
...
class FuzzyMatcher:
    ...
    def token_sort_ratio(a: str, b: str) -> float:
        ...
        return difflib.SequenceMatcher(None, " ".join(tokenize(a)), " ".join(tokenize(b))).ratio()

    @classmethod
    def best_match(cls, query: str, candidates: List[str], min_score: float = 0.6) -> Optional[Tuple[str, float]]:
        if not candidates:
            return None
        ...
        for cand in candidates:
            ...
            scores.append((cand, max(r, pr, tsr)))
...
class SpecAdapter:
    """
    Production-grade spec adapter with:
    ...
    def __init__(
        ...
        self._fallback_order = [
            AdaptationStrategy.EXACT_MATCH,
            AdaptationStrategy.CANONICAL_MATCH,
            AdaptationStrategy.FUZZY_MATCH,
            AdaptationStrategy.STRUCTURAL,
        ]

    def create_adaptation_plan(
        self, source_spec_dict: Dict[str, Any], target_spec_dict: Dict[str, Any]
    ) -> AdaptationPlan:
        src_errors = SchemaValidator.validate_spec(source_spec_dict, "source")
        tgt_errors = SchemaValidator.validate_spec(target_spec_dict, "target")
        all_errors = src_errors + tgt_errors
        ...
        if all_errors:
            return AdaptationPlan(
                ...
                interface_mappings=[],
                register_mappings=[],
                overall_confidence=MappingConfidence.NONE,
                overall_score=0.0,
                strategy_used=AdaptationStrategy.EXACT_MATCH,
                errors=all_errors,
            )
        best_plan: Optional[AdaptationPlan] = None
        best_score = -1.0
        for strategy in self._fallback_order:
            try:
                plan = self._create_plan_with_strategy(
                    source_spec_dict, target_spec_dict, strategy
                )
                if plan.overall_score > best_score:
                    best_score = plan.overall_score
                    plan.strategy_used = strategy
                    best_plan = plan
                if plan.is_safe() and plan.overall_score >= 0.8:
                    break
            except Exception as e:
                self._logger.warning("Strategy %s failed: %s", strategy.value, e)
        if best_plan is None:
            best_plan = AdaptationPlan(
                source_design_name=source_name,
                target_design_name=target_name,
                interface_mappings=[],
                register_mappings=[],
                ...
                overall_score=0.0,
                strategy_used=AdaptationStrategy.EXACT_MATCH,
                errors=["All adaptation strategies failed"],
            )
        best_plan.preview = self._generate_preview(best_plan)
        self._logger.info(
            "Adaptation plan: %s -> %s | score=%.2f | strategy=%s | errors=%d",
            source_name, target_name, best_plan.overall_score,
            best_plan.strategy_used.value, len(best_plan.errors),
        )
        return best_plan

    def _create_plan_with_strategy(
        self,
        ...
        if_map = self._map_interfaces(source_interfaces, target_interfaces, strategy)
        reg_map = self._map_registers(source_registers, target_registers, strategy)
        ...
        for ifm in if_map:
            for sm in ifm.signal_mappings:
                if sm.is_width_mismatch:
                    warnings.append(
                        f"Signal width: {sm.source_name}({sm.source_width}) -> "
                        f"{sm.target_name}({sm.target_width})"
                    )
                if sm.is_direction_mismatch:
                    warnings.append(
                        f"Signal direction: {sm.source_name}({sm.source_direction}) -> "
                        f"{sm.target_name}({sm.target_direction})"
                    )
        if overall_score < 0.5:
            errors.append(f"Confidence too low: {overall_score:.2f}")
    ...
    def _generate_preview(self, plan: AdaptationPlan) -> AdaptationPreview:
        preview = AdaptationPreview(
            source_design=plan.source_design_name,
            target_design=plan.target_design_name,
        )
        if plan.source_design_name != plan.target_design_name:
            preview.renames.append(f"{plan.source_design_name} -> {plan.target_design_name}")
        for ifm in plan.interface_mappings:
            for sm in ifm.signal_mappings:
                if sm.is_renamed:
                    preview.signal_changes.append(
                        f"{sm.source_name} -> {sm.target_name} (conf={sm.confidence_score:.2f})"
                    )
                if sm.is_width_mismatch:
                    preview.signal_changes.append(
                        f"{sm.target_name} width: {sm.source_width} -> {sm.target_width}"
                    )
        for rm in plan.register_mappings:
            if rm.source_name != rm.target_name:
                preview.register_changes.append(
                    f"{rm.source_name} -> {rm.target_name} (conf={rm.confidence_score:.2f})"
                )
    ...
    def _map_interfaces(
        ...
        for src_name, src_iface in source_by_name.items():
            ...
            if strategy == AdaptationStrategy.EXACT_MATCH and src_name in target_by_name:
                best_match = (src_name, 1.0)
            elif strategy == AdaptationStrategy.CANONICAL_MATCH:
                src_canon, _ = SignalCanonicalizer.canonicalize(src_name, self.source_protocol)
                for tgt_name in target_by_name:
                    if tgt_name in matched_target:
                        continue
                    tgt_canon, _ = SignalCanonicalizer.canonicalize(tgt_name, self.target_protocol)
                    if src_canon and tgt_canon and src_canon == tgt_canon:
                        best_match = (tgt_name, 0.95)
                        break
            elif strategy in (AdaptationStrategy.FUZZY_MATCH, AdaptationStrategy.STRUCTURAL):
                candidates = [n for n in target_by_name if n not in matched_target]
                result = FuzzyMatcher.best_match(src_name, candidates, min_score=0.5)
                if result:
                    best_match = result
            if best_match:
    ...
    def _map_signals(
        ...
        for src_name, src_sig in src_sigs.items():
            ...
            src_canon, _ = SignalCanonicalizer.canonicalize(src_name, self.source_protocol)
            tgt_canon_map: Dict[str, str] = {}
            for tn in tgt_sigs:
                tc, _ = SignalCanonicalizer.canonicalize(tn, self.target_protocol)
                tgt_canon_map[tn] = tc
            candidates: List[Tuple[str, float, str, str, int, str]] = []
            for tgt_name, tgt_sig in tgt_sigs.items():
                ...
                tgt_canon = tgt_canon_map.get(tgt_name, "")
                score = 0.0
                reason = ""
                ...
                if strategy == AdaptationStrategy.EXACT_MATCH and match_strategy != "exact":
                    continue
                if strategy == AdaptationStrategy.CANONICAL_MATCH and match_strategy not in ("exact", "canonical"):
                    continue
                ...
                candidates.append((tgt_name, final_score, reason, tgt_dir, tgt_width, match_strategy))
            if candidates:
                candidates.sort(key=lambda x: x[1], reverse=True)
                best_name, best_score, best_reason, best_dir, best_width, best_strat = candidates[0]
                if best_score >= 0.3:
    ...
    def _map_registers(
        self, source_regs: List[Dict[str, Any]], target_regs: List[Dict[str, Any]],
        strategy: AdaptationStrategy,
    ...
    def _map_fields(self, src_fields: List[str], tgt_fields: List[str]) -> Dict[str, Tuple[str, float]]:
    ...
    def _compute_overall_confidence(
        self, if_maps: List[InterfaceMapping], reg_maps: List[RegisterMapping]
    ...
    def _find_unmapped_source_signals(self, source_ifaces: List[Dict[str, Any]], if_maps: List[InterfaceMapping]) -> List[str]:
        all_src: Set[str] = set()
        for iface in source_ifaces:
            for sig in iface.get("signals", []):
                all_src.add(sig["name"])
        mapped: Set[str] = set()
        for ifm in if_maps:
            for sm in ifm.signal_mappings:
                mapped.add(sm.source_name)
        return sorted(all_src - mapped)

    def _find_unmapped_target_signals(self, target_ifaces: List[Dict[str, Any]], if_maps: List[InterfaceMapping]) -> List[str]:
        all_tgt: Set[str] = set()
        for iface in target_ifaces:
            for sig in iface.get("signals", []):
                all_tgt.add(sig["name"])
        mapped: Set[str] = set()
        for ifm in if_maps:
            for sm in ifm.signal_mappings:
                mapped.add(sm.target_name)
        return sorted(all_tgt - mapped)

    def _find_unmapped_source_registers(self, source_regs: List[Dict[str, Any]], reg_maps: List[RegisterMapping]) -> List[str]:
        all_src = {r["name"] for r in source_regs}
        mapped = {rm.source_name for rm in reg_maps}
        return sorted(all_src - mapped)

    def _find_unmapped_target_registers(self, target_regs: List[Dict[str, Any]], reg_maps: List[RegisterMapping]) -> List[str]:
        all_tgt = {r["name"] for r in target_regs}
        mapped = {rm.target_name for rm in reg_maps}
        return sorted(all_tgt - mapped)

    def apply_adaptation(
        self, plan: AdaptationPlan, source_content: str
        ...
        if old_name != new_name:
            ...
            for pattern, replacement in patterns:
                new_c, count = re.subn(pattern, replacement, content)
                if count > 0:
                    changes.append(f"Renamed {old_name} -> {new_name} ({count}x)")
                    content = new_c
        for ifm in plan.interface_mappings:
            for sm in ifm.signal_mappings:
                if sm.is_renamed and sm.confidence_score >= 0.7:
                    new_c, count = re.subn(rf'\b{re.escape(sm.source_name)}\b', sm.target_name, content)
                    if count > 0:
                        changes.append(f"Signal {sm.source_name} -> {sm.target_name} (conf={sm.confidence_score:.2f}, {count}x)")
                        content = new_c
                    if sm.is_width_mismatch:
                        warnings.append(f"Width mismatch: {sm.source_name}({sm.source_width}) -> {sm.target_name}({sm.target_width})")
        for rm in plan.register_mappings:
            if rm.source_name != rm.target_name and rm.confidence_score >= 0.6:
                new_c, count = re.subn(rf'\b{re.escape(rm.source_name)}\b', rm.target_name, content)
                if count > 0:
                    changes.append(f"Register {rm.source_name} -> {rm.target_name} (conf={rm.confidence_score:.2f}, {count}x)")
                    content = new_c
    ...
    def adapt(
        ...
        plan = self.create_adaptation_plan(source_spec, target_spec)
        if plan.is_safe():
            adapted_files: Dict[str, str] = {}
            for fname, content in source_files.items():
                adapted, fchanges, fwarns = self.apply_adaptation(plan, content)
                adapted_files[fname] = adapted
                for c in fchanges:
                    plan.warnings.append(f"[{fname}] {c}")
                for w in fwarns:
                    plan.warnings.append(f"[{fname}] {w}")
            plan.adapted_files = adapted_files
        return plan
